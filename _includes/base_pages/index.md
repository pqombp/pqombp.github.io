<figure class="text-center">
    <img src="/assets/images/turne2025-dag4.jpg" alt="PQ på turné 2025" class="image-frame"/>
    <figcaption class="figure-caption">
        {%- if page.sv -%}
            PQ på turné 2025, Tyskland (foto: Axel Boström)
        {%- else -%}
            PQ on our 2025 tour, Germany (photo: Axel Boström)
        {%- endif -%}
    </figcaption>
</figure>

~.jumbotron.~

{% if page.sv %}

Vi rekryterar!
==============

Ny på KTH? Spelar Du ett instrument och har storbands-/orkestererfarenhet eller dansar Du?
{: .lead}

<a class="btn btn-lg" href="{{ site.dir }}ga-med" role="button">Mer information</a>

Välkomna till våra öppna rep på onsdagar den 21/1 och 28/1 kl 17.45 i Nymble!

{% else %}

We are recruiting!
==================

New to KTH? Do you play an instrument and have bigband/orchestra experience or do you dance?
{: .lead}

<a class="btn btn-lg" href="{{ site.dir }}join-us" role="button">More information</a>

You are welcome to our open rehearsals on Wednesdays 21/01 and 28/01, at 17:45 in Nymble!

{% endif %}

~.~

~.text.~

{%- if page.sv -%}

**Promenadorquestern och med Baletten Paletten** (PQ) är Tekniska Högskolans Studentkårs, vid Kungliga Tekniska högskolan i Stockholm, officiella studentorkester. Vi är en blåsorkester och dansgrupp som spelar jazz, med inslag av både klassiska och mer modernare stilar.

Föreningen känns igen på den gul-svarta utstyrseln med svart frack, vilken är typisk för studentkulturen vid KTH.

Vi har spelningar året om, oftast för företag eller på festivaler. Vi utför både spelningar med hela orkestern eller i mindre grupper som lämpar sig bättre för mindre evenemang. Är du intresserad av att boka PQ till ditt evenemang, se mer på [bokningssidan]({{ site.dir }}boka) och kontakta vår [Directeur](mailto:dirren@pq.se).

Under tiden, lyssna på våra största bangers:

{%- else -%}

**Promenadorquestern och med Baletten Paletten** (PQ) at KTH Royal Institute of Technology is the THS Student Union’s official student orchestra. We are a wind orchestra with a dance group that performs jazz, with elements of both classical and more modern styles.

The association is recognizable by their black and yellow appearance with black tailcoats, which are typical for student life at KTH.

We have shows year round, mostly for companies or at festivals. We perform both with the whole orchestra and dance group, as well as in smaller ensembles that are more appropriate for smaller events. If you interested in booking PQ for your event, please refer to the [booking page]({{ site.dir }}book-us) and contact our [Directeur](mailto:dirren@pq.se).

In the meanwhile, have a listen to our biggest bangers:

{%- endif %}

~.float.right.~

<iframe src="https://embed.spotify.com/follow/1/?uri=spotify:artist:1MuP2VJJAxrHsf4gwJzBU2&size=detail&theme=dark"
        width="100%" height="380" frameborder="0" style="border:none; overflow:hidden;"
        allowtransparency="true"></iframe>
