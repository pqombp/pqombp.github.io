...
`_data/redirects.csv`, and are generated by `_plugins/redirect.rb`. Edit the
CSV file to modify the available redirects.

**Translations** are implemented as follows. The user writes a file in
`_includes/base_pages` that includes both Swedish and English translations,
toggled by Liquid tags:
...
`_data/page_translations.csv`. The plugin `_plugins/page_translations.rb`
reads the CSV file and generates the two versions of the page. Only the
English versions get a subdirectory, `/en/`.
